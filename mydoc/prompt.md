...
--------------
`/data3/zhongzhw/code/uni_ai/google/xla/xla/backends/ascend/transforms/test_hlo.txt` 是需要优化的hlo ir,现在我需要匹配里面的卷积反向计算的相关算子(注意不要错误匹配前向)，然后可以转换成调用： `/data3/zhongzhw/code/uni_ai/ascend/ops/ops-nn/conv/convolution_backward/docs/aclnnConvolutionBackward.md` ，请你根据aclnn-pass-implementer这个技能： `/data3/zhongzhw/code/uni_ai/google/xla/.trae/skills/aclnn-pass-implementer/SKILL.md` 帮我完成功能，原来已经实现了部分功能，比如： `/data3/zhongzhw/code/uni_ai/google/xla/xla/backends/ascend/transforms/aclnn_convolution_backward_rewriter.cc` ，但存在很多的错误，请你重新依据技能帮我实现一遍。
修改完成后需要你执行 `/data3/zhongzhw/code/uni_ai/google/xla/build_ascend_fusion_analyzer.sh` 这个编译脚本，直到编译通过。在编译过程中如果出现错误，请认真解决，遵循功能正确，而不是 为了解决错误，直接裁剪功能。另外每次修改完成后要在同一个终端里执行build.sh，另外如果另外开一个终端编译会导致jax,xla又全量重编，需要大量的时间。
编译成功后，最后执行 `/data3/zhongzhw/code/uni_ai/google/xla/start_test.sh` 这个脚本，这个脚本是测试 `/data3/zhongzhw/code/uni_ai/google/xla/xla/backends/ascend/transforms/aclnn_fusion_analyzer.cc` 这个程序，这个程序会调用你新加的PASS，你需要确认测试结果输出的的IR: `/data3/zhongzhw/code/uni_ai/google/xla/xla/backends/ascend/transforms/aclnn_fusion_analyzer.cc#L85-86` 是正确的，并且正常转换到了aclnnConvolutionBackward这个接口对应的target custom_call指令

---------------
`/data3/zhongzhw/code/google/jax/xla/xla/backends/ascend/transforms/aclnn_convolution_rewriter.cc` 用来处理前向的卷积+bias融合转换为custom_call指令， `/data3/zhongzhw/code/google/jax/xla/xla/service/ascend/thunk_emitter.cc#L4123-4221` 用来将前面转换得到的指令发射为aclnnthunk， `/data3/zhongzhw/code/google/jax/xla/xla/backends/ascend/runtime/aclnn_thunk.cc#L154-188` 这里将aclnnthunk转换为调用 `/data3/zhongzhw/code/google/ascend/ops-nn/conv/convolution_forward/docs/aclnnConvolution.md` ，请你分析aclnnConvolution这个接口，再仔细检查前面三个部分的代码处理是否都是正确的，有没有处理不遗漏或错误的地方