...
int main(int argc, char** argv) {
  ...
  // 获取StreamExecutor（使用第一个设备）
  //const std::vector<se::StreamExecutor*>& stream_executors = backend->stream_executors();
  //if (stream_executors.empty()) {
  //  std::cerr << "No stream executors available" << std::endl;
  //  return 1;
  //}
  //se::StreamExecutor* executor = stream_executors[0];
  se::StreamExecutor* executor = backend->default_stream_executor();
  
  // 第一部分：运行HloPasses进行优化
  std::cout << "Running HloPasses for optimization..." << std::endl;
  ...
  absl::StatusOr<std::unique_ptr<xla::HloModule>> optimized_module_or = 
      backend->compiler()->RunHloPasses(std::move(hlo_module), executor, compile_options);
  if (!optimized_module_or.ok()) {
    std::cerr << "Failed to run HloPasses: " 
              << optimized_module_or.status().ToString() << std::endl;
    return 1;
  }
  std::unique_ptr<xla::HloModule> optimized_module = std::move(*optimized_module_or);
  std::cout << "Successfully completed HloPasses optimization!" << std::endl;
  DumpHloModuleDuringPassIfEnabled("run_hlo_passes", "after", *optimized_module);
  return 0;
}
