...
class StableHloAxpyGpuTest : public ::testing::Test {
 protected:
...
TEST_F(StableHloAxpyGpuTest, UsePjrtCppWrapper) {
  std::unique_ptr<PjRtClient> client = GetCpuClient();
  EXPECT_THAT(client, NotNull());
}

TEST_F(StableHloAxpyGpuTest, RegisterAPIAndRetrieve) {
...
TEST_F(StableHloAxpyGpuTest, CompileCPUTestProgram) {
  std::unique_ptr<PjRtClient> client = GetCpuClient();
...
TEST_F(StableHloAxpyGpuTest, CompileAndExecuteCPUTestProgram) {
